...
int main() {
    ...
    const auto legacy_v6_path = root.string() + ".legacy-source-v6.bin";
    ...
    check(read_pod_at<std::uint64_t>(bytes, channel_count_offset) == coarse_count,
          "v6 persistence stores one channel volume per L0 cell");
    ...
    // Construct an actual v2-shaped day-zero fixture from a v6 writer: no retained channel
    // water exists, so removing only the v6 channel block exactly represents the old semantics.
    ...
    save_multiresolution_water_state(legacy_state, legacy_v6_path);
    auto legacy_bytes = read_bytes(legacy_v6_path);
    ...
    std::filesystem::remove(legacy_v6_path);
